...
Let us denote
$$\mathcal{P}(M)$$ as the set of all mperms of $$M$$.
Then we have
<span>$$|\mathcal{P}(M)| =  {n \choose m_{1},m_{2},\ldots ,m_{l}}$$</span>
as the number of mperms for the multiset $$M$$.
...
<span>$$R(p) := |\{q \in M | q \lt p\}|$$</span>
as the number of preceding mperms.
...
we can see that $$(m, p^*) \lt p$$ as these start with a smaller element than $$p$$.
The number of mperms that can be formed from the remaining elements $$\{p_2^*, ..., p_n^*\}$$ is known by its multinomial coefficient.

This enables us to count all mperms of $$M$$ that start with an element less than $$p_1$$.
