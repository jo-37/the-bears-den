...
---
### Solution
This week's challenge produced some failures.
...
#### First Attempt: Group indices
My first attempt exploited the order of indices and their multitude.
...
The process' efficiency could be improved if we were able to efficiently compare the target positions of a pair of "indices" and apply some `sort` based on the pairwise compare.

Taking an index a the pair $$(p, i)$$ with $$p$$ as the step number and $$i$$ as the insertion index, 
