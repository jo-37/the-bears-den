...
The core of this implementation are little tricks on bits.
...
Then its binary representation is a single one-bit followed by zero or more zero-bits.
...
- $$n$$ is represented as a sequence of arbitrary bits, followed by a zero-bit and zero or more one-bits.
  ...
  Then $$\sim n$$ has the arbitrary bits flipped, followed by a one-bit and zero or more zero-bits,

  $$\sim n$$:

  |y|...|y|1|0|...|0|

  while $$n + 1$$ has the zero-bit flipped to one, the following (if any) one-bits flipped to zero
and the leading arbitrary bits remain unchanged.
  ...
  Taking into account that $$x \& y = 0$$, we find $$(n + 1)\,\&\,\sim n = 2^k$$
